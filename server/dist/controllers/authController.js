...
export const registerUser = async (req, res) => {
    ...
    try {
        ...
        const refreshToken = jwt.sign({ userId: user._id, sessionId: session._id }, process.env.REFRESH_JWT_SECRET, { expiresIn: "7d" });
        ...
        if (!process.env.ACCESS_JWT_SECRET)
            throw new Error("key doesnt exist");
        const accessToken = jwt.sign({ userId: user._id, sessionId: session._id }, process.env.ACCESS_JWT_SECRET, { expiresIn: "15m" });
...
export const rotateToken = async (req, res) => {
    try {
        ...
        const newRefreshToken = jwt.sign({ userId: decoded.userId, sessionId: decoded.sessionId }, process.env.REFRESH_JWT_SECRET, { expiresIn: "7d" });
        if (!process.env.ACCESS_JWT_SECRET)
            throw new Error("Internal server error");
        const newAccessToken = jwt.sign({ userId: decoded.userId, sessionId: decoded.sessionId }, process.env.ACCESS_JWT_SECRET, { expiresIn: "15m" });
        const RTSalt = await bcrypt.genSalt(10);
